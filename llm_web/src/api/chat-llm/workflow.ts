...
import { searchAllHotels } from "./hotel-database";

const MODEL = "gemini-2.0-flash-001";
const DEFAULT_QUERY = "Find me a cheap hotel for tomorrow";
...
const GraphState = Annotation.Root({
  ...
  searchResults: Annotation<HotelSearchResult>({
    reducer: (current, update) => update ?? current,
    default: () => ({} as HotelSearchResult),
  }),
...
async function parseQuery(state: typeof GraphState.State) {
  ...
  const systemPrompt = `You are a hotel search assistant. ${
    hasContext
      ? "Continue this conversation by"
      : "Parse the user's natural language query and"
  ...
  ${
    hasContext
      ? "Use the conversation history to fill in missing information. If the user is providing additional details (like dates), combine them with previous information."
      : "If any required information is missing, ask for clarification."
  }
    ...
    if (parsed.needsClarification) {
      const newConversationHistory = [
        ...state.conversationHistory,
        {
          ...
          content: parsed.clarificationMessage,
          timestamp: new Date(),
        },
      ];
...
async function searchHotels(state: typeof GraphState.State) {
  console.log("🏨 [LangGraph Node] Searching hotels...");
  ...
  try {
    const searchResults = await searchAllHotels(state.query);

    return {
      searchResults,
      analysis: `Found ${searchResults.totalResults} hotels`,
    };
  } catch {
    return {
...
async function analyzeAndSelectCheapest(state: typeof GraphState.State) {
  console.log(
    "💰 [LangGraph Node] Analyzing prices and selecting cheapest option..."
  );
  ...
  // Collect all rooms from all
  const allRooms: HotelRoom[] = state.searchResults?.rooms;
  ...
  const userPrompt = `Here are the hotel search results:
    ...
    ${allRooms
      .map(
        (room) =>
          `- ${room.hotelName} (${room.roomType}): $${room.price}/night - ${room.rating}/5`
      )
      .join("\n")}
...
function shouldContinueToSearch(state: typeof GraphState.State): string {
  ...
  if (state.error || state.needsUserInput) {
    console.log(
      "❌ [LangGraph Router] Stopping workflow - error or needs user input"
    );
    return END;
  }
...
function shouldContinueToAnalysis(state: typeof GraphState.State): string {
  console.log(
    "🔀 [LangGraph Router] Checking if should continue to analysis..."
  );
  if (state.error) {
...
export async function findCheapestHotel(
  ...
  const initialState = {
    ...
    searchResults: {} as HotelSearchResult,
    analysis: userQuery,
